...
public class Contests extends AbstractController {
	...
	public static void finish(String examKey, boolean sendMail) throws MalformedURLException, EmailException {
		Exam exam = Exam.findByKey(examKey);
	...
	private static void sendMail(Exam exam) throws EmailException, MalformedURLException {
		...
		examinerEmail.setSubject("Exam fininsh for " + exam.candidate.firstname + " " + exam.candidate.lastname);
		examinerEmail.setHtmlMsg("<html><body><p><a href=\'http://localhost:9000/exam/show/" + exam.examKey
				+ "\'>Exam finished</a></p></body></html>");
		// set the alternative message
		examinerEmail.setTextMsg("Exam finished");
		Mail.send(examinerEmail);

		System.out.println("Mail has been sent to " + exam.candidate.email + " for finisnhed contest " + exam.examKey);
	...
	public static void nextQuestion(String examKey, String simpleResponse, String response1, String response2,
			...
			Boolean correct4, Boolean correct5) throws Exception {
		beginQuiz(examKey);
	}
